...
func (p *agentService) run() {
	...
	backoff := utils.NewExponentialBackoff(time.Second, 2*time.Minute)

	for {
		select {
		case <-p.ctx.Done():
			p.logger.Info("Context done. Exiting service...")
			return
		default:
			p.logger.Info("Attempting to establish WebSocket connection...")

			client, err := websockets.NewWSClient(func(c *websocket.Conn, m websockets.Message) {
				controllers.WSHandler(p.ctx, c, m, infoChan, errChan)
			})
			if err != nil {
				errChan <- fmt.Sprintf("failed to create WebSocket client: %s", err)
			}

			if err := client.Connect(); err != nil {
				errChan <- fmt.Sprintf("failed to connect WebSocket: %s", err)
			} else {
				backoff.Reset()
			}
			...
			p.logger.Errorf("Connection failed: %v, retrying in %v...", err, delay)
