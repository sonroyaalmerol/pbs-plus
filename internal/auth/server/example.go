...
func New(config *Config) (*Server, error) {
	...
	tokenManager, err := token.NewManager(token.Config{
		...
		CleanupInterval: config.TokenCleanupInterval,
	})
	if err != nil {
...
func (s *Server) handleBootstrap(w http.ResponseWriter, r *http.Request) {
	...
	token, err := s.tokens.GenerateToken(req.AgentID)
	if err != nil {
...
func (s *Server) handleSecure(w http.ResponseWriter, r *http.Request) {
	...
	agentID, err := s.tokens.ValidateToken(token)
	if err != nil {
	...
	if req.AgentID != agentID {
		http.Error(w, "agent ID mismatch", http.StatusUnauthorized)
		return
	}
